...
router.post('/', async (req, res) => {
  ...
  if (couponId) {
    const coupon = await db.prepare('SELECT * FROM coupons WHERE id = ?').get(couponId)
  ...
  for (const item of items) {
    await itemStmt.run(orderId, item.productId, item.productName, item.productImage, item.specName, item.price, item.quantity)
  }

  if (couponId) {
    await db.prepare('UPDATE user_coupons SET status = "used", used_at = CURRENT_TIMESTAMP WHERE user_id = ? AND coupon_id = ?').run(req.userId, couponId)
  }

  await db.prepare('DELETE FROM carts WHERE user_id = ? AND selected = 1').run(req.userId)
...
router.get('/:id', async (req, res) => {
  const db = getDb()
  const order = await db.prepare('SELECT * FROM orders WHERE id = ? AND user_id = ?').get(req.params.id, req.userId)
...
router.put('/:id/cancel', async (req, res) => {
  const db = getDb()
  await db.prepare('UPDATE orders SET status = "cancelled" WHERE id = ? AND user_id = ? AND status = "pending"').run(req.params.id, req.userId)
